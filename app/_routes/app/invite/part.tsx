...
export const loader = async ({
	...
	const errorRedirect = _createErrorRedirect(session, "/app/student")
			...
			select: {
				...
				wallet: {
					select: {
						...
						accountantStudents: { select: { name: true } },
					},
				},
			},
		})
...
export default ({
	loaderData: { part, isBelonging, isLeader },
						...
						<Note>
							...
							<span>
								{part.wallet.accountantStudents.length > 0
									? part.wallet.accountantStudents
...
export const action = async ({
	...
	const student = await verifyStudent(session)
	const errorRedirect = _createErrorRedirect(session, "/app/student")
	const successRedirect = _createSuccessRedirect(session, "/app/student")
	...
	if (action === "leave") {
		const part = await prisma.part
			.update({
				where: { id: partId },
				data: {
					students: { disconnect: { id: student.id } },
					leaders: { disconnect: { id: student.id } },
				},
				select: { name: true },
			})
	...
	if (action === "leader-leave") {
		const part = await prisma.part
			.update({
				where: { id: partId },
				data: { leaders: { disconnect: { id: student.id } } },
				select: { name: true },
			})
