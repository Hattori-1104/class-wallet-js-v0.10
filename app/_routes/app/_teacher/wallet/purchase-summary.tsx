...
export const loader = async ({
	...
	if (!partId || partId === "all") {
		...
		const purchases = wallet.parts
			...
			.sort(
				(a, b) =>
					a.state.receiptSubmission!.receiptIndex -
					b.state.receiptSubmission!.receiptIndex,
			)
		return {
			walletId: wallet.id,
	...
	const part = await prisma.part
		.findUniqueOrThrow({
			where: { id: partId, wallet: { id: walletId } },
			select: {
				id: true,
				name: true,
				budget: true,
...
export default ({
	loaderData: { walletId, parts, budget, purchases },
	...
	const handleDisplayChange = (value: string) => {
		navigate(`/app/teacher/wallet/${walletId}/purchase/?partId=${value}`, {
			replace: true,
		})
	}
				...
				<TableBody>
					...
					{purchases.map((purchase, index) => (
						<TableRow key={purchase.id}>
							...
							<TableCell className="text-center">
								{purchase.state.receiptSubmission!.at.toLocaleDateString(
									"ja-JP",
									{
										month: "numeric",
										day: "numeric",
