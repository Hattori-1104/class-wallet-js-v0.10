...
export const loader = async ({ request }: Route.LoaderArgs) => {
	...
	const belongingPartsPromise = prisma.part
		.findMany({ where: { students: { some: { id: student.id } } }, select: { id: true, name: true } })
		.catch(errorRedirect("パートの取得に失敗しました。").catch())

	const accountingWalletsPromise = prisma.wallet
		.findMany({ where: { accountantStudents: { some: { id: student.id } } }, select: { id: true, name: true } })
		.catch(errorRedirect("ウォレットの取得に失敗しました。").catch())

	const [belongingParts, accountingWallets] = await Promise.all([belongingPartsPromise, accountingWalletsPromise])

	return { belongingParts, accountingWallets }
}

export default ({ loaderData: { belongingParts, accountingWallets } }: Route.ComponentProps) => {
	return (
		<>
			<HorizonContainer>
				<Section>
					...
					<div className="space-y-4">
						{belongingParts.length > 0 ? (
							belongingParts.map((part, index) => (
								<Link key={index} to={`/app/student/part/${part.id}`} className="block">
									<LightBox>
					...
					<div className="space-y-4">
						{accountingWallets.length > 0 ? (
							accountingWallets.map((wallet, index) => (
								<Link key={wallet.id + index} to={`/app/student/wallet/${wallet.id}`} className="block">
									<LightBox>
