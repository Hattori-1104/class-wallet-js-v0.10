...
export default ({ actionData, loaderData: { wallet } }: Route.ComponentProps) => {
	const [form, fields] = useForm({
		lastResult: actionData,
		onValidate({ formData }) {
			return parseWithZod(formData, { schema: NewPartSchemaBuilder(wallet.budget) })
		},
...
export const action = async ({ request, params: { walletId } }: Route.ActionArgs) => {
	...
	const wallet = await prisma.wallet
		...
		.catch(errorRedirect("ウォレットが見つかりません。").catch())
	const result = parseWithZod(await request.formData(), { schema: NewPartSchemaBuilder(wallet.budget) })
