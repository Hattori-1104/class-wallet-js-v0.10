...
type PurchaseSectionProps = {
	wallet: Prisma.WalletGetPayload<{
		select: {
			parts: {
				select: {
					id: true
...
export const PurchaseSection = ({ wallet }: PurchaseSectionProps) => {
		...
		<Section>
			...
			<div className="flex flex-col gap-4">
				{wallet.parts.map((part) =>
					part.purchases.map((purchase) => <PurchaseBlock key={purchase.id} purchase={purchase} />),
				)}
			</div>
		</Section>
	)
...
type PurchaseBlockProps = {
	purchase: PurchaseSectionProps["wallet"]["parts"][number]["purchases"][number]
}
