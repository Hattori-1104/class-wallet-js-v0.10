import { TeacherApproval, parseTeacherApprovalAction } from "~/components/page/purchase/teacher-approval"
...
const serverCommon = async ({ request, params: { walletId, purchaseId } }: Route.LoaderArgs) => {
	...
	const isInCharge = purchase.part.wallet.teachers.some((t) => t.id === teacher.id)
...
export const action = async (actionArgs: Route.ActionArgs) => {
	...
	const successRedirect = createSuccessRedirect(session, `/app/teacher/wallet/${walletId}/purchase/${purchaseId}`)
				...
				data: {
					state: {
						update: {
							teacherApproval: {
								upsert: {
									create: { approved: true, by: { connect: { id: teacher.id } } },
				...
				data: {
					state: {
						update: {
							teacherApproval: {
								upsert: {
									create: { approved: false, by: { connect: { id: teacher.id } } },
