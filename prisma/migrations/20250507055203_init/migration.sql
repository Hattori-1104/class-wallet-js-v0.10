...
CREATE TABLE `PurchaseStateGivenMoney` (
    `id` VARCHAR(191) NOT NULL,
    `teacherApprovalId` VARCHAR(191) NOT NULL,
    `amount` INTEGER NOT NULL,
    `at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),

    UNIQUE INDEX `PurchaseStateGivenMoney_teacherApprovalId_key`(`teacherApprovalId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `PurchaseStateGivenMoney` ADD CONSTRAINT `PurchaseStateGivenMoney_teacherApprovalId_fkey` FOREIGN KEY (`teacherApprovalId`) REFERENCES `PurchaseStateTeacherApproval`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
